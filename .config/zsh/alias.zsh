...
alias dv='cd ~/works/repos/dev'
alias pj='cd ~/works/repos/pj'
...
alias gbc="git pull --prune && git branch --merged | grep -v \$(git rev-parse --abbrev-ref HEAD) | xargs --no-run-if-empty git branch -d" # マージ済みのブランチを削除 cf. https://e-penguiner.com/remove-local-branches-not-on-remote/
